...
def train(net, train_img, train_lbls, test_img):
		max_epochs=2
		n_batches=169
		train_size=1352
		batch_size=8
		...
		for i in range(max_epochs):
			...
			for j in range(n_batches):
				...
				#residual += loss.item()
				print('epoch: %d' %i, 'batch: %d' %j, 'loss: %f' %loss)
				residual=0
...
prediction= train(net, train_img, train_lbls, test_img) # 1 in prediction for Collar and 0 for V_neck
